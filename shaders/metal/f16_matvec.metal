...
// F16 TILED matvec — float16, native half type
...
kernel void f16_matvec(device const half  *weights [[buffer(0)]],
    ...
    if (global_row < rows) {
        for (uint b = 0; b < blocks_per_row; b++) {
            ...
            for (uint i = 0; i < ELEMS_PER_THREAD; i++) {
                ...
                acc += float(weights[w_base + elem_base + elem]) * x[x_base + elem_base + elem];
            }
        }
    }
