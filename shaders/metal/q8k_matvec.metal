...
constant uint BLOCK_BYTES = 292;

kernel void q8k_matvec(device const uchar *weights [[buffer(0)]],
    ...
    if (global_row < rows) {
        ...
        for (uint bi = 0; bi < n_blocks; bi++) {
            ...
            float d = *(device const float *)block;
            device const char *qs = (device const char *)(block + 4);
            uint elem_base = bi * 256;
            uint my_start = local_elem * ELEMS_PER_THREAD;
            for (uint i = 0; i < ELEMS_PER_THREAD; i++) {
                uint elem = my_start + i;
                acc += d * float(qs[elem]) * x[x_base + elem_base + elem];
            }
        }
    }
