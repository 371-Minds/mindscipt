...
kernel void tq2_matvec(device const uchar *weights [[buffer(0)]],
    ...
    float acc = 0.0f;
    if (global_row < rows) {
        uint row_byte = global_row * n_blocks * 66;
        for (uint bi = 0; bi < n_blocks; bi++) {
            device const uchar *block = weights + row_byte + bi * 66;
            device const uchar *qs = block;
            float d = float(*(device const half *)(block + 64));
            ...
            for (uint i = 0; i < ELEMS_PER_THREAD; i++) {
                ...
                uint shift = (elem % 4) * 2;
                int trit = int((qbyte >> shift) & 3) - 1;
                acc += d * float(trit) * x[x_base + elem_base + elem];
            }
        }
    }
