...
// p0 = qkv_dim, p1 = kern (typically 4)
...
fn main(@builtin(workgroup_id) wid: vec3<u32>,
    ...
    for (var k: u32 = 0u; k < kern - 1u; k++) {
        sum += conv_state[k * qkv_dim + ch] * conv1d_w[ch * kern + k];
    }
    let cur = qkv[ch];
    ...
    for (var k: u32 = 0u; k < kern - 2u; k++) {
        conv_state[k * qkv_dim + ch] = conv_state[(k + 1u) * qkv_dim + ch];
    }
    conv_state[(kern - 2u) * qkv_dim + ch] = cur;
